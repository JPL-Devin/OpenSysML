...
// plain association's Links::Link or Links::BinaryLink (KerML §8.4.4).
func TestImplicitKerMLAssocStructBaseIsLinkObject(t *testing.T) {
	m, root := buildModelNamed(t, "t.kerml", `package Objects { struct Object; struct LinkObject :> Object; assoc struct BinaryLinkObject :> LinkObject; }
	...
	p := sym(t, root, "P")
	want := map[string]string{"S": "BinaryLinkObject", "S3": "LinkObject", "A": "BinaryLink", "A3": "Link"}
	for name, base := range want {
		pkg := "Objects"
		if base == "Link" || base == "BinaryLink" {
			pkg = "Links"
		}
		baseSym, _ := sym(t, root, pkg).Scope.LookupLocal(base)
		decl, _ := p.Scope.LookupLocal(name)
		if supers := m.DirectSupertypes(decl); len(supers) != 1 || supers[0] != baseSym {
			t.Errorf("DirectSupertypes(%s) = %v, want [%s::%s]", name, supers, pkg, base)
		}
	}
}

...
func TestQuantityCalculationsReport(t *testing.T) {
	...
	cases := []struct {
		...
		{"VectorCalculations::transform(1, (1.0, 2.0))", ErrUnevaluableLibraryFunction, "VectorCalculations::transform: a CoordinateTransformation has no representation: the runtime holds a scalar measurement reference but no coordinate frame with an origin and basisDirections"},

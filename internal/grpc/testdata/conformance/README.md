...
| `expected_slots` | Instantiate | slot name → `{materialized, value_kind, value}` |
...
`int_value`, `real_value`, `bool_value`, `string_value` or `null`. The assertion checks the
oneof arm as well as the payload, so a value returned with the wrong type fails.

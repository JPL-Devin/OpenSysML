- **A specialization or redefinition that inherits a result expression may not state a second.**
  ...
  `calc def D :> C { x + 2 }` and a calculation or constraint usage typed by a definition that
  owns a body are rejected with `Only one (owned or inherited) result expression is allowed`, on
  the newly stated body, as the reference validators reject them; two generals each owning a

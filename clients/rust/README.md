...
enum, unset value, complex number, array, vector, vector quantity, or
feature-value arm, the client understands that answer. Consumers can inspect
`Capabilities::has` or use `Capabilities::require` when they need to gate their
own use of `enum_values`, `unset_value`, `complex_values`, `structured_values`,

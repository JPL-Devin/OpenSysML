...
- Java stubs are generated by `buf` from a plugin entry in the root `buf.gen.yaml` with the
  version pinned inline, regenerated by `make proto`, and a `java-test` CircleCI job runs the
  tests and the suite while the Go job checks the committed stubs are what `buf` generates.

...
**Test coverage:** 3,585 tests and subtests (3,580 pass, 5 skip themselves; 1,928 top-level `Test` functions) covering parsers, semantics, runtime (actions, states, instances, operators, validation). Behavioral robustness: 80 golden ASTs, 127 negatives, 209 conformance cases, 67 golden traces, 143 robustness subtests.
...
**Behavioral execution:** Calc/constraint/requirement/satisfy fully functional. Action/state executors complete with nested invocation, control flow keywords, loop and conditional statements, send statement (209/209 conformance tests passing). See [SPEC_COMPLIANCE.md](docs/SPEC_COMPLIANCE.md) for measured compliance (~98% faithful implementation).

...
A download records its release tag and digest beside the binary
(`~/.pysysml/bin/sysml-grpc.json`), so a cache left by an earlier release is
replaced instead of being served to a client asking for a newer one — otherwise
an old build answers and the call fails as a `MissingCapabilityError` naming a
capability the requested release does have. Asking for a release is what

...
- 292 runtime test functions (`grep -c '^func Test' internal/core/runtime/*_test.go`), the conformance, trace and robustness gates above among them
...
- 2,724 tests and subtests, of which 2,719 pass and 5 skip (`go test -race -count=1 -v ./...`; 1,511 top-level `Test` functions). The skips are unimplemented-expression and requirement-subject cases that skip themselves.

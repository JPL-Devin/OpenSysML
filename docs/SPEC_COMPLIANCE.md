...
| A redefining feature is the feature it redefines, so `part subsystems : Component[*] :>> Subsystems` makes both names read one collection (KerML 1.0 §7.3.4.5) | `subsetting.go` `aliasRedefinedSlots` | conformance `cubesat_mass_rollup` | ✅ Faithful |

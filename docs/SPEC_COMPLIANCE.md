...
| An output bound twice by one activation — by its declaration and by an assignment, or by two assignments — is a typed error rather than a silent pick (the precedent of a feature valued two ways) | `runtime/calc_statements.go` `calcStmtHost.assignOuter` (`ErrConflictingOutput`) | `robustness_test.go:testCalcOutputValuedAndAssigned`, `:testCalcOutputAssignedTwice` | ✅ Faithful |

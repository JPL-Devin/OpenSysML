...
| Transition effect actions, whether written as a statement (`do assign x := 1`) or as a performed action (`do perform Bump`) | `lower/state_graph.go` `lowerEffect` (the membership a performed action is contributed through is unwrapped, so the executor is handed the action); `state_executor.go:535` fireTransition, executeAction | `state_transition_effect.sysml`, `state_transition_effect_perform.sysml` conformance | ✅ Faithful |

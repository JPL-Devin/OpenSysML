...
| Unqualified library names in files that do not import their library (`Boolean`, `Real`, `that`) | — (deliberate: the root namespace owns the library packages, not their members, so `Integer` resolves only through an import — KerML 8.2.3.2, 8.2.3.5) | `resolve/suggest.go` `unresolvedMessage` (the diagnostic names the qualified spelling), `model/suggestion_test.go`, `model/examples_test.go` | ⚠️ Approximate (an unqualified library name is genuinely unresolved and every surface now says which qualified name was meant; KerML implicit features such as `that` are still not modelled) |
...
**Reference:** docs/API.md § "SysML v2 API & Services `Query`" documents the property table,

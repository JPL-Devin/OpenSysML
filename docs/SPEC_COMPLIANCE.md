...
| One run per usage, object and bound input tuple: two reads from one enclosing invocation run the body once, two invocations binding different values do not share it. A usage binding an output of its own reads the enclosing environment for that binding, which is part of the key as well, so two invocations whose inputs coincide still get their own values | `runtime/calc_usage.go` `calcUsageKey`/`calcUsageRun`/`bindsOwnOutput`, `calc_usage_key.go` `calcInputsKey`/`calcEnvKey` (values rendered by kind and content, collections element-wise, a set by its sorted elements) | `calc_usage_nested_in_calc.trace.golden`, `calc_usage_nested_shadowed_input.trace.golden`, `calc_usage_nested_test.go:TestNestedCalcUsageRunsPerInputs`, `:TestNestedCalcUsageOwnOutputPerInvocation`, `calc_usage_key_test.go` | ✅ Faithful |

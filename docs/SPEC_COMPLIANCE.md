...
|-----|---------------|--------|-------|
| GetServerInfo | service.go `Service.GetServerInfo`, capability names in `Capabilities()` | ✅ Faithful — reports the build version (informational; a source build reports `dev`) and the capabilities this build supports by name, currently `type_facts` and `convert`. A capability is added, never renamed or dropped with its behaviour intact, so a client requires one instead of comparing versions; a service predating this RPC answers `UNIMPLEMENTED`, which the client reads as supporting no capability | service_test.go:TestGetServerInfo, `TestGetServerInfoTypeFactsCapabilityIsHonest`, python/tests/test_capabilities.py |
| ParseFile | service.go `Service.ParseFile` (parser + passes.Analyze + stdlib load) | ✅ Faithful — the cache is keyed by the hash of the content the service read, so repeated parses of unchanged content hit it whatever the request's (ignored) `content_hash` says, and a hash disagreeing with its content cannot serve another model; a record parsed under a different file name is not reused, since its diagnostics name that file | runtime_test.go:TestParseFile_*, service_test.go:`TestParseFileCachesByContentRead`, every conformance case |
| Convert | export.go `Service.Convert`, conversion in `internal/core/export` | ✅ Faithful for what Systemica writes — SysML/KerML notation and RDF Turtle, from a path or inline content, with the format names `sysml -convert` takes and canonical names reported back. Notation to notation is source-preserving (comments and layout survive); a graph direction returns an equivalent model, not identical bytes, and drops comments, per RDF_INTEROP.md. A conversion that cannot be written faithfully returns `error` plus the diagnostics rather than partial output, and `tolerate_syntax_errors` is honored for notation to notation only, since a graph built from an unparsed declaration would lose it silently | export_test.go:TestConvert*, python/tests/test_conversion.py |

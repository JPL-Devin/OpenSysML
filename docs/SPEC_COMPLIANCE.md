...
| Step budget bounds calc evaluation | `context.go` step counter (`ErrStepLimitExceeded`), budget from `budget.go` `MaxStepsFromEnv` (`SYSML_MAX_STEPS`, default 100000) | `robustness_test.go:testStepBudgetExceeded`, `budget_test.go:TestMaxStepsFromValue` | ✅ Faithful |
...
| A legitimately long loop runs under a raised budget | `budget.go` `MaxStepsFromEnv` (`SYSML_MAX_STEPS`) resolved at the REPL/CLI and gRPC entry points | `budget_test.go:TestRaisedBudgetRunsLongerLoop` | ✅ Faithful (a 10 000-iteration loop that exhausts the default budget completes under a raised one) |
...
| Step budget enforcement | `context.go` incrementStep; budget configured by `SYSML_MAX_STEPS` (`budget.go` `MaxStepsFromEnv`) | `robustness_test.go:testStepBudgetExceeded`, `budget_test.go:TestRaisedBudgetRunsLongerLoop` | ✅ Faithful (the reported limit is the effective one, and names the variable that raises it) |

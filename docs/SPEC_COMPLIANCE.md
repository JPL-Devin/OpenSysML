...
- 59 golden AST fixtures (including body-local calc usages and ranges, the three loop forms, pseudostate, timed-trigger, call-trigger, calc default/invocation, calc statement bodies and n-ary connector-end parsing tests)
...
| A feature bound to a value takes its own features from that value, so a body restating one of them (`attribute :>> ringCost = 400.0 { attribute :>> v = 9.0; }`) states two values for it and is reported | `instance.go` `restatedInValuedBody`, `GetSlot` (`ErrValuedFeatureRestated`) | `robustness_test.go:valued_feature_restated_in_a_body` | ✅ Faithful (a typed error rather than a silent pick between the value and the body) |

...
| A usage declared among a calc's members binds its inputs in the environment of the evaluation reading it — the enclosing calc's parameters and locals, then the enclosing lexical scope (SysML 7.17, a usage is a feature of the body declaring it) | `runtime/calc_usage.go` `bindCalcUsage`/`enclosedByCalc`, `eval.go` `EvalContext.nestedEnv`, `invoke_calc.go` `bindCalcParameters` | `calc_usage_nested_in_calc.sysml`, `calc_usage_nested_test.go:TestNestedCalcUsageBindsFromEnclosingParameters`, `:TestNestedCalcUsageBindsFromEnclosingLocals`, `:TestNestedCalcUsageChain`, `:TestNestedCalcUsageReadsEnclosingObject` | ✅ Faithful |
...
| A calc usage declared in a loop or conditional body is executable, in the scope it is written in and with the lifetime of the block: an iteration binds it from that iteration's state and reading it again in the same iteration reuses that evaluation | `lower/calc_body.go` `usageStatement` → `lower.DeclareUsage`, `runtime/statements.go` `declareUsage`, `runtime/calc_usage.go` `bodyUsageSymbol` | `calc_body_local_usage_and_range.sysml` (golden AST), `calc_rk4_lunar_descent.sysml`, `calc_usage_body_local_test.go:TestBodyLocalCalcUsageInLoopBindsPerIteration`, `:TestBodyLocalCalcUsageInBranch`, `:TestBodyLocalCalcUsageInNestedBodies`, `robustness_test.go:testBodyLocalUsageOfANonCalc`, `:testBodyLocalDeclarationNotExecutable` | ✅ Faithful |
| A feature chain through a part reads what the part's features carry, the part being materialized as the occurrence it denotes; an output of a calc usage the part declares evaluates in that part's context | `runtime/eval.go` `evalFeatureChain`/`chainBase`/`occurrenceOperand`, `runtime/instance.go` `occurrenceOf` | `part_feature_chain_test.go:TestCalcUsageReadThroughPartChain`, `:TestCalcUsageChainDiagnostics`, `robustness_test.go:testUsageReadThroughAPartWithoutAnOutput` | ✅ Faithful |
...
| `expose` in a view body is an Import | see the Name Resolution section's `expose` rows | `parser/expose_test.go`, `resolve/expose_test.go`, `parse/view_expose.golden` | ⚠️ Approximate (`validateExposeOwningNamespace` is still not implemented: an `expose` outside a view body is parsed and resolved rather than diagnosed — roadmap A5) |

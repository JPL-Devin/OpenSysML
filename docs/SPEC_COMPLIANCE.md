...
- 250 runtime test functions (`grep -c '^func Test' internal/core/runtime/*_test.go`), the conformance, trace and robustness gates above among them
...
| A nested input bound from a name of its own (`in vx = vx`) reads the enclosing binding, the frame it is written into being pushed under the enclosing environment rather than over the name it resolves | `runtime/calc_usage.go` `bindCalcUsage`, `eval.go` `EvalContext.nestedEnv`/`Lookup` | `calc_usage_nested_shadowed_input.sysml`, `calc_usage_nested_test.go:TestNestedCalcUsageShadowsEnclosingName`, `robustness_test.go:testNestedCalcUsageSelfCycle` (a default with nothing outside to name stays `ErrCyclicSlot`) | ✅ Faithful |
| One run per usage, object and bound input tuple: two reads from one enclosing invocation run the body once, two invocations binding different values do not share it | `runtime/calc_usage.go` `calcUsageKey`/`calcUsageRun`, `calc_usage_key.go` `calcInputsKey` (values rendered by kind and content, collections element-wise, a set by its sorted elements) | `calc_usage_nested_in_calc.trace.golden`, `calc_usage_nested_shadowed_input.trace.golden`, `calc_usage_nested_test.go:TestNestedCalcUsageRunsPerInputs`, `calc_usage_key_test.go` | ✅ Faithful |
| A nested usage chain is bounded: the depth counted while an output binding is evaluated, the budget spent by the bodies it runs | `invoke_calc.go` `enterCalc` (`ErrCalcRecursionLimit`), `runtime/calc_usage.go` `calcRun.value` | `robustness_test.go:testNestedCalcUsageRecursionDepth`, `:testNestedCalcUsageStepBudget`, `:testNestedCalcUsageUnboundInput`, `:testNestedCalcUsageUnknownOutput` | ✅ Faithful |

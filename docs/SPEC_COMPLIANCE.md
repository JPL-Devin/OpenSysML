...
**Test Coverage:**
- 53 conformance cases (all passing: calc×10, constraint×3, requirement×5, action×8, state×26, accept×1)
...
| N-ary connector ends (`connection link connect (a, b, c)`), SysML v2 7.13.2, 8.3.13 | `parser/defusage.go` `parseConnectorEnds` (parenthesized end list); `passes/constraint.go` `checkConnectorEnds` (arity by kind); `lower/connection.go` `lowerConnections`, `PeerPorts` | `parse/connection_nary.golden`, `parser/connector_ends_nary_test.go` `TestParseNaryConnectorEndsKeepsEveryEnd`, `parser/negative_test.go` (`nary_connect_unclosed`, `nary_connect_trailing_comma`, `nary_connect_empty`), `passes/constraint_test.go` `TestConstraintConnectionNaryEndCountReachesTheChecker`, `lower/connection_test.go` `TestLowerNaryConnectionKeepsEveryEnd`, `conformance/action_port_communication_nary.sysml` | ⚠️ Approximate (a *named* connection, connector, interface or allocation keeps every end of a parenthesized list end to end — parse, constraint tier, lowering and port routing — and an interface or allocation beyond two ends is reported. The **anonymous inline** form `connect (a, b, c);`, which declares no connection name, is binary-only: `parser/defusage.go` parses it in `parseBodyMember` rather than through `parseConnectorEnds` and reports `expected connector end after 'connect'`) |
...
| A private `import X::*` is not re-exported by its namespace (KerML 8.2.3.3) | `symbols/index.go` `markReexported` / `exportedChildren` (a re-export a private import produced is recorded as hidden and left out when that namespace is itself wildcard-imported) | `symbols/index_test.go` `TestExpandWildcardImportsDoesNotCarryOnAPrivateImport` | ⚠️ Approximate (the privately imported name is not carried on to a package importing the namespace, but is still registered under the namespace's own FQN, so a direct qualified reference to it resolves — `resolve/alias.go` `resolveCachedAliasTarget` relies on that to follow an alias of a privately imported type) |

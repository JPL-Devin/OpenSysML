...
| The budget bounds one run, not a session | `context.go` `beginRun`/`startRunIfTopLevel` (the step counter is reset when a run begins; a nested run shares the outer one's budget) | `budget_test.go:TestStepBudgetIsPerRun` | ✅ Faithful |

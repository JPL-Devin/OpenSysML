...
- 2,813 tests and subtests, of which 2,808 pass and 5 skip (`go test -count=1 -v ./...`; 1,543 top-level `Test` functions). The skips are unimplemented-expression and requirement-subject cases that skip themselves.
...
| A `variant` whose owner is not a variation offers no choice, so it stays an ordinary feature of its owner and the idle `variant` keyword is reported | `passes/constraint.go` `checkVariantOutsideVariation` (warning `variant-outside-variation`), `runtime/shape.go` `buildFeatures` and `runtime/eval.go` (only a variant of a variation is a choice rather than a slot, via `semantics/variation.go` `VariationOwning`) | `variant_outside_a_variation.sysml`, `robustness_test.go:variant_outside_a_variation`, `passes/constraint_test.go:TestConstraintVariantOutsideVariation`, `TestConstraintVariantInsideVariationOK` | ✅ Faithful |

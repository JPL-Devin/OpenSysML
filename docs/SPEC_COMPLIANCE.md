...
**Test Coverage:**
- 35 conformance cases (all passing: calc×10, constraint×3, requirement×5, action×5, state×12)
- 22 robustness tests (deadlock, guards, budgets, sourceless accept, fork/join and region-local pseudostate misuse, non-numeric time trigger, misaddressed send, accept of an unsent type, calc binding/arity/recursion failures)
- 41 unit tests
- 22 golden AST fixtures (including pseudostate, timed-trigger and calc default/invocation parsing tests)
- 17 golden execution traces (fork/join branch ordering, send/accept, calc and constraint evaluation)
...
**Test Counts:**
- Conformance cases: 35 (all passing)
- Robustness tests: 22 (all passing)
- Unit tests: 41 (action/state executors)
- Golden AST fixtures: 22
- Golden execution traces: 17
...
- Conformance: 35/35 cases passing

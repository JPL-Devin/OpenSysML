...
**Test Coverage:**
- 160 conformance cases (all passing: calc×47, calcUsage×6, constraint×11, requirement×12, satisfy×5, action×37, state×32, instance×10 — the calc cases include the fixed-step RK4 lunar descent whose stages are body-local usages read over a range, and the one-binding output case)
...
- 287 runtime test functions (`grep -c '^func Test' internal/core/runtime/*_test.go`), the conformance, trace and robustness gates above among them

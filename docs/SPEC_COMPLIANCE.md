...
| A body that both assigns an output and returns a value keeps the two apart: the return is the invocation's value, the assignment the output's, so reading the output answers what the body assigned | `runtime/calc_usage.go` `runCalcUsage` (memoizing a returned value under the designated output's name only when that output is the result parameter or states no assignment of its own) | `calc_output_assigned_in_body.sysml` conformance (`AssignAndReturn`: `cr.a` is 6, `AssignAndReturn(5)` is 500) | ✅ Faithful |

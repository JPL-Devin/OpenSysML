...
- **What is not mapped is refused, not partly converted**, and the refusal names
  ...
  268, differs as a graph for 1, and 6 cannot be written back or re-read at all.
...
**A head the writer spells itself comes back in one spelling.** The graph
carries what a head declares, not how it was spelled, so where no stored text
is written — the graph carries none, or its text no longer states the graph —
the notation is normalised where the notation offers a choice and the model
does not:

- A relationship written as a symbol or as its keyword (`:>` or `subsets`,
  `:>>` or `redefines`, `::>` or `references`) is the same relationship
  element, so no spelling is recorded and the writer uses one form; a stored
  text keeps the author's, since the two compare equal. This differs from
  `sysx:declaredKeyword`, which is kept where the notation's synonyms name
...
the second hop reproduces the Turtle byte for byte for 268 of the 275 files that
convert and differs for the rest ([rdf-corpus-roundtrip.md](../project/rdf-corpus-roundtrip.md)). An end

...
- **What is not mapped is refused, not partly converted**, and the refusal names
  the construct. 287 of the 345 models under `examples/` (committed, training and
  pilot corpora) convert to Turtle; the other 58 are refused. Of the 287, a second
  conversion of the written-back notation reproduces the graph for 275 (190
  byte-for-byte, 85 up to the whitespace inside `sysx:sourceText`), differs for
  7, and 5 cannot be written back or re-read at all. These figures are the
...
the second hop reproduces the graph exactly for 190 of the 287 files that
convert, up to `sysx:sourceText` whitespace for 85 more, and differs for the

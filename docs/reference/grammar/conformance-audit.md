...
`<source> then <target>;`, `done <name>;` and the orthogonal-region member
`region <name> { … }` are no longer accepted. Use
`succession first <source> then <target>;`, `done;`, and a `parallel` state body
with one state substate per region instead. The state-machine spellings
`initial <state>;` and `transition [<name>] <src> to <tgt>;` are no longer
accepted either: write `entry; then <state>;` and

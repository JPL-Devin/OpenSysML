...
- **Identifiers** — plain, plus **unrestricted** (backtick-quoted) names with escape
  handling.
- **`::`** namespace separator.
- **Numbers** — integer, real, exponent forms.
- **Strings.**
- **Operators & punctuation** — the full KerMLExpressions set.
- **Comments** — `//` line and `/* */` block, plus `comment`/`doc` keyword bodies.
- **Whitespace** — skipped, but tracked for spans.

Every token carries a byte offset + length; line/column is derived via the source line
index. The lexer is UTF-8 aware (Unicode identifiers). **Contextual keywords** (e.g.
`end`, `variation`, `individual`) are emitted as identifiers and disambiguated at the
parser level — the lexer stays dumb and fast. An illegal character produces an **error
token** and the lexer continues (recovery-friendly).

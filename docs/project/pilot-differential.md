...
| `examples` | 33 | 25 | 2 | 573 | 0 | 1 | 1 | 572 |
| `cmd/pilot-diff/testdata` (probes) | 4 | 1 | 6 | 0 | 0 | 0 | 6 | 0 |
| **Total** | **367** | **338** | **56** | **634** | **34** | **2** | **20** | **598** |
...
`examples` 10 syntax, 15 `unmapped`, 260 `kind-mismatch`, 287 `unresolved-reference` — of which
...
**`oosem-demo/oosem-demo.sysml` adds 296 pilot-only rows with the same single cause: the reference
...
specialization the reference cannot resolve cascades into 106 `unresolved-reference` and 190
`kind-mismatch` (`Must invoke a behavior or a behavioral feature` and its metadata-keyword
cousins, 9 of them warnings). The `examples` only-pilot column moves 302 → 598 while only-ours
...
| overall: pilot diagnostics | 617 | **634** |
| `examples`: agreed diagnostics | 9 | **0** |
| `examples`: our diagnostics | 11 | **2** |
| `examples`: pilot diagnostics | 556 | **573** |

...
| `examples/pilot-corpora/sysml-examples` | 98 | 84 | 63 | 0 | 0 | 0 | 63 | 0 |
| `examples/pilot-corpora/sysml-validation` | 56 | 53 | 3 | 0 | 0 | 0 | 3 | 0 |
| `examples/pilot-corpora/kerml-examples` | 58 | 50 | 4 | 6 | 0 | 0 | 4 | 6 |
| `testdata` | 15 | 8 | 29 | 53 | 25 | 1 | 3 | 27 |
| `examples` | 22 | 15 | 86 | 63 | 0 | 23 | 63 | 40 |
| `cmd/pilot-diff/testdata` (probes) | 4 | 1 | 6 | 0 | 0 | 0 | 6 | 0 |
| **Total** | **353** | **311** | **191** | **122** | **25** | **24** | **142** | **73** |

**Read the `only ours` total by root, never as one number.** It rises 119 → 142 in wave 10 while our
diagnostics against the reference's **own** corpora fall 82 → **70** — `pilot-examples` 68 → 63,
`pilot-validation` 10 → 3, `kerml-examples` unchanged at 4. The whole increase is in the `examples` root, which is *our* demo models,
and all 63 of its rows are one check firing on them: the non-standard-notation warning (26
`then <source> <target>;`, 24 `transition <source> to <target>;`, 6 `require` outside a requirement
body, and 7 across `done`/`initial`/`region`/`junction`). **Those are true positives about our own
examples, not candidate false positives about our implementation** — the column header is wrong for
them, and the honest count of suspect diagnostics of ours against the reference corpora is 70. It also
explains `severity-only` 15 → 24 and only-pilot 85 → **73** in the same root: where the pilot errors on
a line we now warn on, the pair moves out of only-pilot into severity-only rather than either side
changing what it detects.

One genuine new false positive of ours is in the rise, and it is not in that root:
`pilot-examples/Vehicle Example/Annex_A_VehicleViews.sysml` gains 8 rows, two groups of four
`Duplicate of inherited member name '<x>' from Vehicle, vehicle_b` — the same feature reached through
two supertypes counted as a duplicate, where the pilot is silent. Against it, the same root's
`SysML v2 Spec Annex A SimpleVehicleModel.sysml` falls 24 → 10.

This table is a clean full run on `main` at `07dc713c`, with wave 10 merged — #450 (9A implicit
...
Per category, the only-ours totals are: `pilot-examples` 36 `unresolved-reference`, 17 `unmapped`,
9 `kind-mismatch`, 1 `units`; `pilot-validation` 2 `kind-mismatch`, 1 `unresolved-reference`;
`kerml-examples` 3 `unmapped`, 1 `unresolved-reference`; `examples` 63 syntax; `testdata` 2

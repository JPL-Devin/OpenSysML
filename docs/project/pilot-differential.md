...
written against, "F60–F69" the round of #358–#364, "wave 3" the round of #372–#376, and "now"
the wave-4 round of #383/#391/#387/#388/#382:

| Count | At #356 | After F60–F69 | After wave 3 | Now | Reason for the wave-4 move | Reason for the wave-3 move |
|---|---|---|---|---|---|---|
| overall: fully agreeing / only ours / our diagnostics | 221 / 560 / 596 | 254 / 343 / 379 | 273 / 281 / 317 | **283 / 232 / 268** | Wave 4, and it is the largest single-round drop in only-ours since F60–F69: −49 diagnostics and +10 fully-agreeing files, from #391 (resolution, −22 `unresolved-reference`), #383 (F65 parser forms) and #387 (F99). Agreement (20) and severity-only (16) are **unchanged for the third round running** — every round so far removes diagnostics of ours rather than converging them onto the reference's. | #374 (KerML grammar F50, F70, F81–F83 and the F62/F63 leftovers), #375 (F64/F66/F71), #372 (symbols and typecheck leftovers). Agreement (20) and severity-only (16) are unchanged again — no fix in either round has turned a disagreement into an agreement, which is worth noticing: our diagnostics disappear rather than converging on the reference's. |
...
The KerML row is still the harshest of the three OMG roots and it is almost entirely ours: 72
only-ours against 6 only-pilot, with 39 of 58 files fully agreeing (439 / 6 and 10 / 58 when the
...
| F101 | Wave-4 handback from F68 (#391): 11 of its 39 diagnostics remain, all implicit `Actions::TransitionAction` members (`accepter`, `effect`, `acceptedMessage`, `receiver`). `semantics/implicit.go` `kindBaseFQN` must give `*ast.TransitionMember` the base its usage kind already has — a layer #391 did not own. |
...
agreeing with 72 diagnostics of ours, against `pilot-examples` at 76 of 98 and `pilot-validation`
at 52 of 56. The SysML side of the corpora is in much better shape than the KerML side, and the
claim the numbers support today is about SysML parsing, not KerML.

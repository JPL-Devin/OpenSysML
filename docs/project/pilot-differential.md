...
| `examples/pilot-corpora/sysml-examples` | 98 | 70 | 138 | 0 | 0 | 0 | 138 | 0 |
| `examples/pilot-corpora/sysml-validation` | 56 | 50 | 22 | 0 | 0 | 0 | 22 | 0 |
| `examples/pilot-corpora/kerml-examples` | 58 | 37 | 80 | 6 | 0 | 0 | 80 | 6 |
| `testdata` | 11 | 3 | 27 | 54 | 20 | 4 | 3 | 30 |
| `examples` | 22 | 12 | 44 | 118 | 0 | 12 | 32 | 106 |
| `cmd/pilot-diff/testdata` (probes) | 4 | 1 | 6 | 0 | 0 | 0 | 6 | 0 |
| **Total** | **349** | **273** | **317** | **178** | **20** | **16** | **281** | **142** |

This table is a clean full run on `main` at `fa602fd6`, with the F60–F69 round (#358–#364) and
the round after it (#372 symbols/typecheck, #373 fixtures, #374 KerML grammar, #375 behavior
parser, #376 F21–F23) merged, and it is the run the committed baseline was regenerated from. Two
consecutive runs produce byte-identical JSON, so the numbers are not order- or timing-dependent.

Per category, the only-ours totals are: `pilot-examples` 75 syntax, 56
`unresolved-reference`, 4 `unmapped`, 2 `kind-mismatch`, 1 `units`; `pilot-validation` 20
syntax, 2 `kind-mismatch`; `kerml-examples` 70 syntax, 6 `unresolved-reference`, 4 `unmapped`;
...
diagnostics in the pre-merge comparison, and 22 now.
...
written against, "F60–F69" the round of #358–#364, "now" the round of #372–#376:

| Count | At #356 | After F60–F69 | Now | Reason for the latest move |
|---|---|---|---|---|
| overall: fully agreeing / only ours / our diagnostics | 221 / 560 / 596 | 254 / 343 / 379 | 273 / 281 / 317 | #374 (KerML grammar F50, F70, F81–F83 and the F62/F63 leftovers), #375 (F64/F66/F71), #372 (symbols and typecheck leftovers). Agreement (20) and severity-only (16) are unchanged again — no fix in either round has turned a disagreement into an agreement, which is worth noticing: our diagnostics disappear rather than converging on the reference's. |
| `pilot-examples`: only ours | 314 | 167 | 138 | syntax 93 → 75 (#375's `return`-as-usage and expression-body declarations). `kind-mismatch` 20 → 2 is #372: a keyword-less or metadata-prefixed usage is a `ReferenceUsage`, so the attribute-typing and conjugated-port rules must not apply to it — that retires the whole Arrowhead cluster. `unresolved-reference` rises 51 → 56, unmasking again (F99). |
| `pilot-validation`: only ours | 59 | 37 | 22 | syntax 34 → 20; `kind-mismatch` 3 → 2. |
| `kerml-examples`: only ours | 150 | 98 | 80 | #374 is the KerML-side round: syntax 85 → 70, `unresolved-reference` 10 → 6 (F70 stops reading `rep`/`inOCL`/`language` as names, F71 stops losing a parameter's name). `unmapped` 3 → 4. The open KerML rows are now F72 and F84–F95 — K7–K18, none of which this round claimed. |
| `examples`: only pilot | 109 | 423 | 106 | #373 (**F96**, mislabelled "F84" in that PR's title): all 314 `.kerml` diagnostics are gone — two fixtures were misnamed SysML demos and were renamed, the other eight had their notation corrected to what the KerML grammar accepts. The `.sysml` side moves 109 → 106 with the two renamed files added to it. This root's only-pilot column is now entirely our own SysML demo models being genuinely invalid. |
| `examples`: fully agreeing | 4 | 4 | 12 | #373 again, plus #376's correction of `views-demo.sysml` to dotted flow ends. |
| `unmapped`, our side | 13 | 13 | 16 | The 11 specialization-cycle rows (F4/K5), the conjugated-port `interface Mounting` message and the `Issue::text` name conflict, plus **three** newly-unmasked: the two `Rationale` metadata name conflicts (a deliberate one-sided check of ours, F69, that F64b stopped masking) and `TimeVaryingCarDriver.kerml:93`'s `isLicensed1 redefines isLicensed, but isLicensed is not an inherited member of driver`, which F50 stopped masking and which the reference does *not* report — F100. |
| new checks of ours | — | — | +3 rules | #376 implements F21 (`flow-end-subsetting`), F22 (filter evaluability, in both directions) and F23 (`invocation-not-behavior`), and **adds no only-ours diagnostic to any root** — including the 100-file training corpus, which stays 0/0. F20 was implemented, flooded the training corpus with false positives, and was reverted: it is reported as not implemented rather than claimed. |

The KerML row is still the harshest of the three OMG roots and it is almost entirely ours: 80
only-ours against 6 only-pilot, with 37 of 58 files fully agreeing (439 / 6 and 10 / 58 when the

...
| 1 | `pilot-examples`: `Geometry Examples/VehicleGeometryAndCoordinateFrames.sysml`:38 | `operator '+' combines incommensurable quantities: a dimensionless value and mm (dimension L)` | adjudicated divergence — the same specification-grounded quantity-commensurability family as `Turbojet Stage Analysis.sysml`:25, attributed below | quantity/dimension analysis — retain the warning; parenthesizing `(22/2*25.4 + 110) [mm]` would apply the unit to the whole sum |
...
| 1 | `pilot-examples`: `Analysis Examples/Turbojet Stage Analysis.sysml`:25 | `operator '+' combines incommensurable quantities` | adjudicated divergence — the same specification-grounded quantity-commensurability family as `VehicleGeometryAndCoordinateFrames.sysml`:38, attributed below | quantity/dimension analysis |
...
which 9 are unresolved-reference defects of ours, 7 are adjudicated specialization cycles, 1 is the
adjudicated conjugation warning, 2 are the adjudicated quantity-commensurability family, and 1 is the
adjudicated direct-type conformance error.
...
|---|---|---|---|---|
| `Geometry Examples/VehicleGeometryAndCoordinateFrames.sysml`:38; `Analysis Examples/Turbojet Stage Analysis.sysml`:25 | The pilot is silent on both. OpenSysML warns that Geometry adds a dimensionless value to a length in `22/2*25.4 + 110 [mm]`, and that Turbojet adds L^6 to Θ in `1/(2 * Cp) * V^2 + T_static`. | KerML 1.0 §8.2.5.8.1–8.2.5.8.2 makes `[mm]` a primary bracket expression, so it binds to `110` rather than the additive expression. In Turbojet, `V : VolumeValue` makes `V^2` L^6, `Cp : DimensionOneValue` does not change that dimension, and `T_static : TemperatureValue` is Θ. SysML v2.0 §9.8.9.1 requires addition operands and result to have the same quantity dimension and top-level quantity type, and says an implementation should warn or error for an invalid operation. Both published examples violate that rule as written; the pilot implements no corresponding static check. | **adjudicated divergence** — one quantity-commensurability family; retain both specification-grounded warnings rather than matching pilot silence | quantity/dimension analysis |
...
| `InterfaceUsage_Invalid.sysml.xt:49` | `error: Cannot have more than two ends` | `An interface definition end must be a port.` at :53 | **pilot limitation** | Step 3: SysML v2 §7.14.1 permits interfaces with three or more ends; §7.14.2 and §8.3.14.2 constrain only `BinaryInterface` |
| `InterfaceUsage_Invalid.sysml.xt:78` | `warning: Duplicate of inherited member name 'self' from Part, Port` | word-for-word agreement after Step 3 | **closed** | exactly two-ended binary typing plus positional implicit end redefinition |
| `BindingConnector_Invalid2.sysml.xt:42` | `warning: Bound features should have conforming types` | nothing at that line | **pilot limitation** | Step 3 found no numbered normative constraint for argument-level conformance on the operator expression `rearWheel+1`; ordinary feature endpoints remain checked |

Step 3's independent fresh-cache control at `4b9baf2d` moves Xpect
**1293 / 248 / 30 → 1295 / 248 / 28** (agree / wording-only / disagree), with the interface diamond
and assignment-referent rows recovered and no regression. Differential
(**324 fully agreeing, 83 only ours by roots 0/8/0/3/3/63/6, 66 only pilot**) and rejection
(**116 both reject, 4 pilot-only**) do not move, and no lower-tier fix unmasks a diagnostic.
The universal interface-arity row and the operator-expression binding row are therefore removed
from the obligation count rather than left as work OpenSysML owes. Metadata `p24` remains the one
explicit deferred obligation, blocked on L3 slice B's library record format.

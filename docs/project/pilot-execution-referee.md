...
prints the current ones. State of the 135 committed cases, the original 32, the 62 the
...
`w6d_expr_depth.cases` and the 9 of `coordinate_frames.cases`:

```
agree: 69 · kind-only: 1 · order-only: 0 · disagree: 4
pilot-unevaluated: 45 · pilot-silent: 4 · pilot-error: 2 · ours-error: 2 · both-error: 8
nondeterministic: 0
```

...
known. Five sub-items, in the order to take them. D3.1–D3.3 are done; D3.4 and the
harness remain.
...
2. **Track D** is independent of the rest and can run whenever. Take **D3** first, in its own
   order: **D3.1**–**D3.3** are done (identity, element ids, materialized ownership), leaving
   **D3.4** and then the **D3.5** harness, which is the first thing that can
   ...
   **D8** (the OWL-ontology output profile) is optional and additive: its domain/range gate is
   ...
   conformant behind D2 and D1, so it does not belong ahead of them.

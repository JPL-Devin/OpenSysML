...
A third tag, `opensysml-v*`, publishes the one-off final release of the
`opensysml` PyPI name the client briefly used — see
[The final `opensysml` release](#the-final-opensysml-release).
...
which runs on a tag matching `/^pysysml-v.*/` — for example `pysysml-v0.4.0`.
The first release after the name came back is 0.4.0: the version line carries on
from `opensysml` 0.3.0, which was the same client, so no version number is
reused. Nothing is uploaded from a laptop, and a `v*` core release tag publishes
no package.
...
```bash
python python/scripts/check_version.py --tag pysysml-v0.4.0   # prints 0.4.0
```
...
Where the project does not exist on PyPI yet
(`https://pypi.org/pypi/<name>/json` → 404), a project-scoped token cannot be
created for it. So:
...
```bash
# 1. Declare a pre-release version, e.g. VERSION = "0.4.0rc1"
$EDITOR python/pysysml/_version.py
# 2. Land it, then tag it
git tag -a pysysml-v0.4.0rc1 -m "pysysml 0.4.0rc1" && git push origin pysysml-v0.4.0rc1
```
...
pip install --index-url https://test.pypi.org/simple/ \
            --extra-index-url https://pypi.org/simple/ pysysml==0.4.0rc1
python -c "import pysysml; print(pysysml.__version__)"
```

Then set `VERSION` to the final version and tag `pysysml-v0.4.0`.
...
pip install pysysml==0.4.0
python -c "import pysysml; print(pysysml.__version__)"    # must print 0.4.0
```
...
## The final `opensysml` release

The client was published as [`opensysml`](https://pypi.org/project/opensysml/) up
to 0.3.0, while the client carried the project's name. That name cannot be
deleted and its last version still installs and works, so `pip install opensysml`
would otherwise go on silently handing out that client indefinitely.

`packaging/pypi-opensysml/` is the answer: `opensysml` 0.3.1, a distribution of
the same name whose only module raises `ImportError` naming `pysysml`. Being
above 0.3.0 is what makes resolvers prefer it. It is not a compatibility shim —
it does not re-export `pysysml`, and it declares no dependency on it, since
installing the client as a side effect would keep the old import working.

`pip install opensysml==0.3.0` is the escape hatch while migrating. An exact pin
is the only one that avoids the placeholder whatever version it carries: any
range that does not exclude it (`>=0.3`, `~=0.3.0`, `<1.0`) resolves to it, which
is the whole point.

The same treatment was applied once before, to `pysysml` 0.2.1, when the client
was renamed to `opensysml`; the name is back, so 0.4.0 supersedes that
placeholder as an ordinary release of the client.

It is released by its own tag, which runs the `release-opensysml-placeholder`
workflow:

```bash
git tag opensysml-v0.3.1    # must match the version in packaging/pypi-opensysml/pyproject.toml
git push origin opensysml-v0.3.1
```
...
into a clean virtualenv and **fails if importing `opensysml` succeeds**. A
placeholder that imports cleanly is the alias this release exists not to be.
`python/tests/test_legacy_opensysml_placeholder.py` asserts the same contract
from source on every run.

This is expected to happen exactly once. Nothing further should be published
under that name; a client fix goes to `pysysml`.

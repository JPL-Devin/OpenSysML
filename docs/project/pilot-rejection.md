...
4. **`semantic/` — the pilot validators' named constraints** (46 cases). The pinned
   ...
   reading every constraint's Java implementation and message string: 42 of those 43 cases are
...
```
166 case(s): 154 both reject, 12 only the pilot rejects, 0 only we reject, 0 both accept
...
argument checking of the send-action family — which also added its three cases, for 166 —
closed another (`k34`), leaving 12. Before
...
the default mode the same corpus gives 152 agreements and 14 gaps, which is what `-conformance
default` prints. `-conformance strict` gives 154 and 12. Reserved keywords recovered as declared

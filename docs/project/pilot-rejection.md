...
```
231 case(s): 204 both reject, 19 only the pilot rejects, 8 only we reject, 0 both accept
...
another) with `g68` (a definition nested in an enumeration body); the feature-variability
rules closed two of the gaps (`k11`, an initial `:=` value on a non-variable feature; `s80`, `constant`
on a non-variable usage). Before that source the default-mode gap count was 2
...
the default mode the same corpus gives 201 agreements and 22 gaps, which is what `-conformance
default` prints. `-conformance strict` gives 204 and 19. Reserved keywords recovered as declared
...
All 19 gaps under `-conformance auto` are open, and all of them were opened by the `semantic/`
source and the SysML constraint census: 10 named KerML constraints (`k16`–`k42`) and 3

...
4. **`semantic/` — the pilot validators' named constraints** (44 cases). The pinned
   ...
   reading every constraint's Java implementation and message string: 42 of the 44 cases are
   `.kerml` (`k01`–`k42`); of the two `.sysml` cases, `s80` is a shared constraint whose only
   ...
   variation. Two things this source records
...
What this corpus cannot see: it tests the invalid models we thought to write. **We authored all 165
...
```
165 case(s): 152 both reject, 13 only the pilot rejects, 0 only we reject, 0 both accept
...
| grammar | 80 | 80 | 0 | 0 | 0 |
| semantic | 44 | 31 | 13 | 0 | 0 |
| xpect | 34 | 34 | 0 | 0 | 0 |

The corpus grew from 79 cases to 119 in wave 10G, to 120 with `g60` (an `alias` named by a
keyword), to 163 with the `semantic/` source, and to 165 with `s01` (an enumeration definition
specializing another) and `g61` (a definition nested in an enumeration body). The `semantic/`
source reopened 14 gaps — all of them semantic
...
because we authored all 165 cases ourselves, a small gap count means we ran out of questions we

...
```
231 case(s): 201 both reject, 22 only the pilot rejects, 8 only we reject, 0 both accept
...
another) with `g68` (a definition nested in an enumeration body) took it to 228, and `s48`–`s50` (the
trigger-argument typing rules) to 231, all three both-reject. Before that source the default-mode gap count was 2

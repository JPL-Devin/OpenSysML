...
4. **`semantic/` — the pilot validators' named constraints** (100 cases). The pinned
...
```
228 case(s): 199 both reject, 21 only the pilot rejects, 8 only we reject, 0 both accept
...
228 with the three send-action cases. The KerML constraints in that
...
the default mode the same corpus gives 196 agreements and 24 gaps, which is what `-conformance

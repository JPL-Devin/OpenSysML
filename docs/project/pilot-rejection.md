...
4. **`semantic/` — the pilot validators' named constraints** (55 cases). The pinned
   ...
   legal violating spelling is SysML's `constant` usage prefix. `s01`–`s03` are the three typing
   rules of `TriggerInvocationExpression` — an `after` argument that is not a `DurationValue`, an
   `at` argument that is not a `TimeInstantValue`, a `when` argument that is not Boolean. `cn01`–`cn09` are the
...
120 with `g60` (an `alias` named by a keyword), and to 175 with the `semantic/` source, which
...
the default mode the same corpus gives 149 agreements and 15 gaps, which is what `-conformance
default` prints. `-conformance strict` gives 151 and 13. Reserved keywords recovered as declared

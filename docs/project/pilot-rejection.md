...
4. **`semantic/` — the pilot validators' named constraints** (53 cases). The pinned
   ...
   legal violating spelling is SysML's `constant` usage prefix. `s01` restates a bound feature
   value (`= 1`) from a redefining usage and a redefining definition
   (`validateFeatureValueOverriding`), found by probing `bin/sysml` against the reference
   validator rather than by an Xpect suite. `cn01`–`cn09` are the
...
120 with `g60` (an `alias` named by a keyword), to 172 with the `semantic/` source, which
...
closed one of them (`k33`), leaving 13; `s01` (the feature-value overriding rule) took it to 173
with both implementations rejecting. Before that source the default-mode gap count was 2 of
...
the default mode the same corpus gives 150 agreements and 15 gaps, which is what `-conformance
default` prints. `-conformance strict` gives 152 and 13. Reserved keywords recovered as declared

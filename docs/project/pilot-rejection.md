...
4. **`semantic/` — the pilot validators' named constraints** (101 cases). The pinned
   ...
   [spec-compliance.md](spec-compliance.md). `s47`–`s49` are the three typing rules of
   `TriggerInvocationExpression` — an `after` argument that is not a `DurationValue`, an `at`
   argument that is not a `TimeInstantValue`, a `when` argument that is not Boolean. `cn01`–`cn09`
   are the control-node succession rules of SysML v2 8.3.17:
...
What this corpus cannot see: it tests the invalid models we thought to write. **We authored all 229
...
```
229 case(s): 199 both reject, 22 only the pilot rejects, 8 only we reject, 0 both accept
...
120 with `g60` (an `alias` named by a keyword), and to 228 with the `semantic/` source (100 cases)
...
corpus to 226 with both implementations rejecting, and `s47`–`s49` (the trigger-argument typing
rules) to 229, all three both-reject. Before that source the default-mode gap count was 2
...
the default mode the same corpus gives 196 agreements and 25 gaps, which is what `-conformance
default` prints. `-conformance strict` gives 199 and 22. Reserved keywords recovered as declared
...
because we authored all 229 cases ourselves, a small gap count means we ran out of questions we

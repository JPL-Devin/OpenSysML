...
4. **`semantic/` — the pilot validators' named constraints** (46 cases). The pinned
   ...
   reading every constraint's Java implementation and message string: 42 of the 46 cases are
   `.kerml` (`k01`–`k42`); of the `.sysml` cases, `s01`–`s03` are the three typing rules of
   `TriggerInvocationExpression` — an `after` argument that is not a `DurationValue`, an `at`
   argument that is not a `TimeInstantValue`, a `when` argument that is not Boolean — and `s80`
   is a shared constraint whose only legal violating spelling is SysML's `constant` usage prefix. Two things this source records
...
What this corpus cannot see: it tests the invalid models we thought to write. **We authored all 166
...
```
166 case(s): 153 both reject, 13 only the pilot rejects, 0 only we reject, 0 both accept
...
| grammar | 79 | 79 | 0 | 0 | 0 |
| semantic | 46 | 33 | 13 | 0 | 0 |
| xpect | 34 | 34 | 0 | 0 | 0 |

The corpus grew from 79 cases to 119 in wave 10G, to 120 with `g60` (an `alias` named by a
keyword), to 163 with the `semantic/` source, which reopened 14 gaps — all of them semantic
rules the pilot enforces and we did not (see [Permissiveness gaps](#permissiveness-gaps)); the
named-argument validation that landed alongside closed one of them (`k33`), leaving 13, and to
166 with the three trigger-typing cases (`s01`–`s03`), which we reject as the pilot does. Before
...
because we authored all 166 cases ourselves, a small gap count means we ran out of questions we

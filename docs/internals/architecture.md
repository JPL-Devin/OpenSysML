...
- **Declared-diagnostic silence:** of the 510 declared `errors` rows in the reference's own Xpect suites, we report nothing for 3. 233 we report word-for-word; 248 wording-only and 9 location-only differences are agreement in substance and are not counted as gaps; 0 more we report as a warning and 11 elsewhere in the file ([Xpect oracle](../project/pilot-xpect.md), `go run ./cmd/pilot-xpect`).
...
**Row bookkeeping:** the ✅/⚠️/❌/⛔ status of each of the 719 tracked rules stays in [spec compliance](../project/spec-compliance.md) as a census of our own row list. It moves when rows are rewritten and does not move when an oracle does, so it is not the progress measure.

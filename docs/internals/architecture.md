...
- **Declared-diagnostic silence:** of the 510 declared `errors` rows in the reference's own Xpect suites, we report nothing for 0. 238 we report word-for-word; 248 wording-only and 10 location-only differences are agreement in substance and are not counted as gaps; 0 more we report as a warning and 7 elsewhere in the file ([Xpect oracle](../project/pilot-xpect.md), `go run ./cmd/pilot-xpect`).

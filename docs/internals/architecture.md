...
**Row bookkeeping:** the ✅/⚠️/❌/⛔ status of each of the 750 tracked rules stays in [spec compliance](../project/spec-compliance.md) as a census of our own row list. It moves when rows are rewritten and does not move when an oracle does, so it is not the progress measure.

...
- Measured after the Step 2 resolver round, with a fresh library cache: `353 file(s), 324 fully agreeing; 32 agreed,
  83 only ours, 66 only the pilot's`, JSON totals `openSysMLDiagnostics 139 / pilotDiagnostics
...
but only while the baseline is current. Check that first. As of the wave-12F rebaseline it **is**
current: a live run gives `353 file(s), 324 fully agreeing; 32 agreed, 83 only ours, 66 only the

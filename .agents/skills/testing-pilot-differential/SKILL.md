...
- Measured after the self-model's library-snapshot round, with a fresh library cache: `366 file(s), 334 fully agreeing; 34 agreed,
  20 only ours, 321 only the pilot's`, JSON totals `openSysMLDiagnostics 56 / pilotDiagnostics
  357 / severityMismatch 2`; ~2 min wall, byte-identical across runs *and* after a from-scratch
...
but only while the baseline is current. Check that first. As of the rebaseline that came with the self-model's library-snapshot round it **is**
current: a live run gives `366 file(s), 334 fully agreeing; 34 agreed, 20 only ours, 321 only the

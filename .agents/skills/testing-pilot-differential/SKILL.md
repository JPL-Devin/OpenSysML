...
- Measured after the objective-evaluation round, with a fresh library cache: `367 file(s), 338 fully agreeing; 34 agreed,
  20 only ours, 598 only the pilot's`, JSON totals `openSysMLDiagnostics 56 / pilotDiagnostics
  634 / severityMismatch 2`; ~2 min wall, byte-identical across runs *and* after a from-scratch
...
current: a live run gives `367 file(s), 338 fully agreeing; 34 agreed, 20 only ours, 598 only the

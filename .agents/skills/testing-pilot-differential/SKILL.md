...
- Measured after the Perseverance example round, with a fresh library cache: `360 file(s), 332 fully agreeing; 34 agreed,
  20 only ours, 55 only the pilot's`, JSON totals `openSysMLDiagnostics 56 / pilotDiagnostics
  91 / severityMismatch 2`; ~70 s wall, byte-identical across runs *and* after a from-scratch
...
current: a live run gives `360 file(s), 332 fully agreeing; 34 agreed, 20 only ours, 55 only the

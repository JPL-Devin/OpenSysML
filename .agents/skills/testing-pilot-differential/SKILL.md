...
- Measured after the architecture self-model round, with a fresh library cache: `364 file(s), 335 fully agreeing; 34 agreed,
  20 only ours, 59 only the pilot's`, JSON totals `openSysMLDiagnostics 56 / pilotDiagnostics
  95 / severityMismatch 2`; ~70 s wall, byte-identical across runs *and* after a from-scratch
...
current: a live run gives `364 file(s), 335 fully agreeing; 34 agreed, 20 only ours, 59 only the

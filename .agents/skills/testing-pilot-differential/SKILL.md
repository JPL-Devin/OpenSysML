...
- Measured after the objective-evaluation round, with a fresh library cache: `366 file(s), 338 fully agreeing; 34 agreed,
  20 only ours, 302 only the pilot's`, JSON totals `openSysMLDiagnostics 56 / pilotDiagnostics
  338 / severityMismatch 2`; ~2 min wall, byte-identical across runs *and* after a from-scratch
...
but only while the baseline is current. Check that first. As of the rebaseline that came with the send-argument round it **is**
current: a live run gives `366 file(s), 338 fully agreeing; 34 agreed, 20 only ours, 302 only the

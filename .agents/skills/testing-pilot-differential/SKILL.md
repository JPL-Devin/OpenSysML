...
- Measured after the package-keyword round, with a fresh library cache: `357 file(s), 330 fully agreeing; 37 agreed,
  27 only ours, 68 only the pilot's`, JSON totals `openSysMLDiagnostics 69 / pilotDiagnostics
  110 / severityMismatch 5`; ~70 s wall, byte-identical across runs *and* after a from-scratch
...
package-keyword round it **is**
current: a live run gives `357 file(s), 330 fully agreeing; 37 agreed, 27 only ours, 68 only the

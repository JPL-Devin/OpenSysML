...
- Measured after the feature-value overriding round, with a fresh library cache: `366 file(s), 337 fully agreeing; 43 agreed,
  20 only ours, 303 only the pilot's`, JSON totals `openSysMLDiagnostics 65 / pilotDiagnostics
  348 / severityMismatch 2`; ~2 min wall, byte-identical across runs *and* after a from-scratch
...
but only while the baseline is current. Check that first. As of the rebaseline that came with the feature-value overriding round it **is**
current: a live run gives `366 file(s), 337 fully agreeing; 43 agreed, 20 only ours, 303 only the

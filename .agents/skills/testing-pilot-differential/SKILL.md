...
- Measured after the relay-probe demo round, with a fresh library cache: `359 file(s), 332 fully agreeing; 34 agreed,
  20 only ours, 53 only the pilot's`, JSON totals `openSysMLDiagnostics 56 / pilotDiagnostics
  89 / severityMismatch 2`; ~70 s wall, byte-identical across runs *and* after a from-scratch
...
current: a live run gives `359 file(s), 332 fully agreeing; 34 agreed, 20 only ours, 53 only the

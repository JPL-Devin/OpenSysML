...
- **Additivity.** `go run ./cmd/pilot-diff` must still print the headline the
  committed baseline holds (`358 file(s), 332 fully agreeing; 34 agreed
  diagnostic(s), 20 only ours, 50 only the pilot's` after the standard-notation demo round — read it from the baseline JSON, not from this line, since each

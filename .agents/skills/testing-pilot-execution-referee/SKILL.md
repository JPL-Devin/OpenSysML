...
- **Additivity.** `go run ./cmd/pilot-diff` must still print the headline the
  committed baseline holds (`366 file(s), 335 fully agreeing; 34 agreed
  diagnostic(s), 20 only ours, 242 only the pilot's` after the architecture self-model round — read it from the baseline JSON, not from this line, since each

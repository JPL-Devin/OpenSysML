...
- **Additivity.** `go run ./cmd/pilot-diff` must still print the headline the
  ...
  diagnostic(s), 20 only ours, 333 only the pilot's` after the control-node succession pass joined the self-model — read it from the baseline JSON, not from this line, since each

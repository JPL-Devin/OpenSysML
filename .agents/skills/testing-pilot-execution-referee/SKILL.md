...
- **Additivity.** `go run ./cmd/pilot-diff` must still print the headline the
  ...
  diagnostic(s), 27 only ours, 61 only the pilot's` after the output-initializer rewrite in
  `examples/action-executor-demo.sysml` — read it from the baseline JSON, not from this line, since each

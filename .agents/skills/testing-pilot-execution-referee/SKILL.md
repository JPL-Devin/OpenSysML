...
- **Additivity.** `go run ./cmd/pilot-diff` must still print the headline the
  committed baseline holds (`353 file(s), 317 fully agreeing; 32 agreed
  diagnostic(s), 119 only ours, 66 only the pilot's` after wave 12A — read it from the baseline JSON, not from this line, since each

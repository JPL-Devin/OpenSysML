...
- **Additivity.** `go run ./cmd/pilot-diff` must still print the headline the
  committed baseline holds (`357 file(s), 330 fully agreeing; 37 agreed
  diagnostic(s), 26 only ours, 75 only the pilot's` after the library-inherited-name gap — read it from the baseline JSON, not from this line, since each

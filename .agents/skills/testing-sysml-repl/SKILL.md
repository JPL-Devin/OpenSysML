...
- **A leftover service may be answering 50051 from a path you never built.** A previous session can
  ...
  any client result, run `pgrep -af sysml-grpc` (here `-f` is right: `-x` misses a binary whose
  argv[0] is an odd path), kill the PID you found **by number**, start your own

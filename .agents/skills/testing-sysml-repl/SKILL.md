...
## A/B-ing a parser change against `main`

Parser changes are on every code path, so the highest-value evidence is a **side-by-side run of the
same fixture on two binaries**. Build the baseline out of a worktree so your branch checkout and
`bin/` are untouched:

```bash
export PATH=/usr/local/go/bin:$PATH
git worktree add /tmp/mainwt main
(cd /tmp/mainwt && go build -o /tmp/mainwt/sysml-main ./cmd/sysml)
```

Then a one-liner prints both verdicts per fixture, which reads well on camera:

```bash
for f in a b c; do
  echo "  main  : $(printf "%%load /tmp/t/$f.sysml\n%%quit\n" | /tmp/mainwt/sysml-main | sed -n '2p')"
  echo "  branch: $(printf "%%load /tmp/t/$f.sysml\n%%quit\n" | ./bin/sysml | sed -n '2p')"
done
```

`sed -n '2p'` drops the REPL banner so the diagnostic (or `✓ package X`) is line 1 of the output.
For a regression set the assertion to prove is **parity**, not success: a form that errors on both
builds is unchanged behavior, and that is a pass.

### `redefines` forms and their current status

In a `part def` body only the `= <value>` form and the modifier/typed forms are accepted:

| form | result |
| --- | --- |
| `redefines x = 1;` | parses |
| `redefines parent.value = 100;` | parses |
| `redefines a.b.c.d.e.f.g = 1;` (long chain) | parses **only since PR #94**; earlier a 10-token lookahead cap made it `expected a body member` |
| `portion redefines x = 1;` | parses (separate modifier path, still capped at 10 tokens) |
| `attribute redefines x;` | parses — this is the working bare-specialization spelling |
| `redefines x;` | `error: expected a body member` — pre-existing limitation on `main` too, do not report as a regression |
| `redefines x : SomeType;` / `redefines x[1];` | same pre-existing `expected a body member` |

Semantics worth knowing before you call a value wrong: `redefines parent.value = 100;` does **not**
overwrite the nested slot. `%slots` shows `parent.value` keeping its original value *and* a new
sibling slot `value = 100` on the owner. That holds identically on `main`, so it is pre-existing
runtime behavior of this parse form, not a bug introduced by a parser fix — verify against a
baseline build before filing it.

To prove a long chain resolves rather than merely parsing, the target must actually exist: nest real
parts (`part outer { part middle { … } }`) or nameres reports `unresolved reference: outer` and you
have only tested the syntax layer.

### Fixture gotcha: `examples/state-machine-demo.sysml`

It loads clean (`✓ package StateMachineDemo`) but `%state StateMachineDemo::Vehicle::vehicleStates`
fails with `no initial state found in state machine vehicleStates` — the model declares no initial
transition. That is true on `main` as well. For a runtime state-machine demo use
`internal/repl/testdata/state_debug.sysml` (`%state Debug::Cycle`, then `%advance 1` → `waiting`,
`%advance 9` → `working`).

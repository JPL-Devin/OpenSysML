...
- `%advance` arguments: `abc` → invalid-time error, `-5` → must-not-be-negative, no arg → `usage:`,
  ...
  Note %advance durations accumulate in a debugger-side clock: `Advanced to <clock>` and `%current`'s
  `Time:` are that clock, while `Last event at:` is the executor clock, which only moves when an event fires.

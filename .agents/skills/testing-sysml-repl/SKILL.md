...
rather than printing stale slots. Note an active `%action`/`%state` debugging session is *not* reset
by a submission — it keeps running against the executor built from the older document.
